...
whole map with no tile-server process. See [`docs/USER_GUIDE.md`](docs/USER_GUIDE.md) for Caddy/nginx
config.

**What is bundled as PMTiles (fully self-hostable, works offline):**
...
**What streams live (needs internet — NOT served by your own host), marked 🌐 in the UI:**
...
> MapSplat converts local files to PMTiles so Caddy can serve them; remote services are kept as
> live-streaming references (🌐) and clearly flagged — the export log notes any source that needs
> internet. Pulling remote services *into* the offline PMTiles bundle is a planned, terms-of-service-
> gated feature (Story 18 Stage 3).
